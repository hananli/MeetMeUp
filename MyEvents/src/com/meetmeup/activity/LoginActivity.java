...
public class LoginActivity extends Activity {
	...
	public static final String USER_PROFILE2 = "/picture?type=small";
...
//	public static AsyncFacebookRunner mAsyncRunner = null;
	...
	//	loginButton.setCompoundDrawablesWithIntrinsicBounds(null, null, null, null);
		...
		loginButton.setReadPermissions(Arrays.asList("user_likes",
				"user_status","public_profile","email","user_checkins","friends_checkins","read_friendlists"));
		/*user_checkins
		friends_checkins
		read_friendlists
		manage_friendlists
	...
	private void getUserFacebookData(GraphUser user) {
		if (user != null) {
			...
			//		Toast.makeText(mContext, "gcm = "+gcm_id, Toast.LENGTH_LONG).show();
...
//					mAsyncRunner.request("me/friends", new FriendsRequestListener());
					...
					new LoginAsync(mContext, new LoginListener(), multipart)
					.execute();
					uiHelper.onDestroy();
				} else {
	...
	//This code used for generating SHA1 key which is used in facebook app project.
	void getHashCode() {
		try {
			PackageInfo info = getPackageManager().getPackageInfo(
					"com.meetmeup.activity", PackageManager.GET_SIGNATURES);
			for (Signature signature : info.signatures) {
				MessageDigest md = MessageDigest.getInstance("SHA");
				md.update(signature.toByteArray());
				Log.d("Your Tag SHA1 key for Facebook.",
